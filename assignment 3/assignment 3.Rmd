...
In general, it makes sense to differentiate between the average treatment effect and the average treatment of effect of the treated when the assignment is not fully random as the $ATE$ and $ATET$ will not be the same as a result. 

The average treatment effect on the treated is more applicable when we want to now the effect of a treatment on the treated group. For example, when conduction cost-benefit analysis of a treatment, we are interested to know whether the effect is sufficient given the costs such as in job support programs where we want to know whether the support measures helped the participants to gain a job. Another situation is the effect of medication where we are interested to know whether a certain medicine helped the participants or not. 
...
As the table shows, the characteristics are not fully balanced at important, grade and siblings (but not at a high significance level.)
...
The intercept is significantly different from $\frac{1}{6}$, therefore, children tend to lie either way. The estimations of the treatment effects are not significantly different from the intercept. Hence, the treatment does not influence behaviour too much and still lie. We of course, added robust standard errors because the outcome variable is binar, hence is heteroskedastic by nature. 

We estimated a model only with the treatment effects, with all covariates, with the random covariates and with gender. Gender is significant while the other covariates are not.
...
In the model including gender as a regressors, we can see that gender is significant at $10\%$ and negative. As the gender dummy equals 1 for girlst, we can conclude that there is weakly significant effect and the girls tend to lie less than boys. However, between the treatments there are no significant differences between gender.
...
- they assume that they get something either way - question is did the research tell them that they will not get anything or did they leave the possibility open
- children could have fun lying to children
- maybe children did not like the researcher or they tend to lie to strangers (although research shows more the other way around - find paper)
- competitive environment among children - did the children knew about the participation of other children?
...
add explaination to MDE_target
...
Note: same df used as above to not have to iteratively solve it (only marginal difference)
