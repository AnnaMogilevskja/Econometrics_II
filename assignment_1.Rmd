...
Earnings usually depend on the educational level of individuals which is why it is sensible to estimate the causal effect of schooling on earnings. However, in order to get a unbiased, consistent and efficient OLS estimation the following regularity conditions have to hold:\\
...
So it is essential that $X_i$ is an exogenous regressor. The issue that can arise in the estimation of the causal effect of schooling on earnings is that schooling itself is an endogenous regressors. For instance, the distance to the school, the amount of financial support for schools, the educational background of the individual's parents are all potential determining factors of an individual's education. The distance is often regarded as a determining factor as individuals who live closer to a college or school tend to higher education. Subsidies for schools are meant to improve the educational quality of institutions which is why we can presume that students from highly subsidized school tend to study more and longer. Children whose parents have an academic background statistically study more often than children whose parents have a worker background. All these aspects support the argument that schooling actually is not an exogenous but an endogenous regressors. Consequently, the assumption of the zero conditional mean does not hold and the OLS estimation would be biased and inconsistent. 
...
summary(IV3)
```
For the first instrumental variable estimation, which considers distance as the only instrumental variable, we can see that the first-stage F-test gives us a F-statistic of around 5.65. As this is below 10, this gives us evidence that the instrument is not relevant which is why we will not consider it any further.\\

The second instrumental variable estimation, which considers subsidy as the only instrumental variable, we can see that the first-stage F-test gives us a F-statstic of around 41.2. Hence, there is evidence that the instrument is relevant. Looking at the second-stage regression, we see that schooling is significant and as a positive effect on wage.\\

The third instrumental variable estimation includes distance and subsidy as its instrumental variables and has a F statistic of around 23. However, the Sargan test, which can be applied when the model is overidentified and tests the exogeneity of instruments, cannot reject the null hypothesis as the p-value is around 0.37. This aligns with our previous results as the Sargan test assumes that one of the instrumental variables is valid and tests whether the instrumental variables are jointly valid. Considering, that distance is not a relevant instrumental variable, it is not surprising that the Sargant test does not reject the null hypothesis.\\
...
Going further, we also estimate instrumental variable estimations where we include age and squared age. Although the numbers change across the estimations, the estimations also show that distance is not a valid instrumental variable while subsidy is. Further, the Sargan test does not reject the null hypothesis in any of the estimations. Therefore, this supports our choice of subsidy as the instrumental variable.\\
...
summary(IV4)
```
Our first OLS model includes schooling, age and age squared as regressors. Schooling is highly significant and has a positive effect on earnings. Age and age squared have a negative effect on wages but are not significant. The F test is around 604.3 so the estimator are jointly significant. The adjuste $R^2$ is around 0.8135. 
...
Given our results, we would prefer the IV estimation as there is evidence for the endogeneity of schooling. Further, we would choose the estimation that also includes the exogenous regressors age and age squared as the F statistic is higher than in the estimation where we only included schooling.\\
